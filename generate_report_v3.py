"""
DDCSE Professional Covenant Surveillance Report — v3.0
Bloomberg / Morgan Stanley institutional aesthetic
Eight-company portfolio: CHTR · WBA · PARA · HCA · ATUS · BHC · M · LUMN
Three NEW sectors added: Specialty Pharma (BHC) · Department Store Retail (M) · Enterprise Telecom (LUMN)

Data: SEC EDGAR 10-K / 10-Q static registry.
...
def build_cover(story, today, cases):
    ...
    story.append(Table([[
        Paragraph('DYNAMIC DEBT COVENANT<br/>SURVEILLANCE ENGINE  ·  v3.0',
    ...
    story.append(Table([[
        Paragraph('Portfolio Covenant Surveillance Report  ·  Private Credit Edition  ·  8 Credits  ·  3 New Sectors',
    ...
    story.append(Table([[
        ...
        Paragraph(f'<b>Prepared By</b><br/>DDCSE Engine v3.0 · github.com/DogInfantry', S['body']),
...
def build_framework(story):
    ...
    story.append(Table([[
        Paragraph(
            '⚑  <b>Data Note (v3.0):</b> Yahoo Finance API was unavailable in the report generation '
            'environment (HTTP 403 — network egress restriction). All financial figures are sourced '
            'from the DDCSE SEC EDGAR static registry with field-level source citations. '
            'v3.0 adds three new sectors: Specialty Pharma (BHC), Department Store Retail (M), '
            'and Enterprise Fiber Telecom (LUMN) — expanding the portfolio from 5 to 8 credits.',
...
# MAIN BUILD
# ─────────────────────────────────────────────────────────────────────────────
def build_new_sector_section(story):
    """Section 3: New sector companies with a banner callout."""
    story.append(Table([[
        Paragraph('3.  NEW Sectors — Specialty Pharma  ·  Department Store Retail  ·  Enterprise Fiber Telecom',
    ...
    story.append(Table([[
        Paragraph(
            'v3.0 expansion: Three new surveillance credits added across three previously uncovered sectors. '
            'BHC (Bausch Health) and LUMN (Lumen Technologies) are in active covenant breach; '
            "M (Macy's) is on watchlist due to acquisition risk and earnings trajectory.",
            ParagraphStyle('nb', fontName='Helvetica', fontSize=8.5, textColor=DGRAY,
    ...
    story.append(Spacer(1, 3 * mm))
    for i, ticker in enumerate(['BHC', 'M', 'LUMN'], 1):
        build_company_page(story, ticker, i)


def build_original_sector_section(story):
    """Section 4: Original 5 companies."""
    story.append(Table([[
        Paragraph('4.  Original Portfolio — Deep-Dive Profiles',
                  ParagraphStyle('s4h', fontName='Helvetica-Bold', fontSize=13,
                      textColor=WHITE, leading=18)),
    ]], colWidths=[CONTENT_W], style=TableStyle([
        ('BACKGROUND', (0,0), (-1,-1), NAVY),
        ('TOPPADDING', (0,0), (-1,-1), 9),
    ...
    story.append(Paragraph(
        'Five-company original surveillance portfolio · Media/Cable · Healthcare Retail · Entertainment · Hospitals · Telecom',
        S['small']))
    story.append(Spacer(1, 3 * mm))
...
def build_expanded_methodology(story, today):
    """v3 methodology section with updated source table for 8 companies."""
    section_header_block(story, '7. Methodology, Data Sources & Disclaimer')

    story.append(Paragraph('Engine Methodology', S['h3']))
    story.append(Paragraph(
        'DDCSE v3.0 implements a deterministic, audit-logged covenant surveillance pipeline across '
        'eight credits spanning six industry verticals. v3.0 extends the original five-company '
        'portfolio with three new sector additions: Specialty Pharmaceuticals (Bausch Health), '
        'Department Store Retail (Macy\'s), and Enterprise Fiber/Telecom (Lumen Technologies). '
        'Covenant rules are compiled into typed Python dataclasses using an AST-safe compiler '
        '(no eval() or exec() calls), ensuring production safety. Severity scores are computed '
        'as continuous 0–100 values using a piecewise linear function of breach depth / threshold '
        'proximity. Portfolio scores are the probability-weighted maximum across all covenants. '
        'All financial data is sourced from the DDCSE SEC EDGAR static registry with per-field '
        'source attribution to specific filing sections and credit agreement clauses.',
        S['body_j']))
    story.append(Spacer(1, 3 * mm))

    story.append(Paragraph('Data Sources — All 8 Credits', S['h3']))
    disc_data = [
        [Paragraph('<b>Ticker</b>', S['white_bold']),
         Paragraph('<b>Company</b>', S['white_bold']),
         Paragraph('<b>Sector</b>', S['white_bold']),
         Paragraph('<b>Filing</b>', S['white_bold']),
         Paragraph('<b>CIK</b>', S['white_bold']),
         Paragraph('<b>Covenant Source</b>', S['white_bold'])],
        ['CHTR', 'Charter Communications', 'Media & Cable', '10-K FY2023 (Feb 2024)', 'CIK 0001091882', 'Charter Operating Credit Agreement (2021) §7.1'],
        ['WBA',  'Walgreens Boots Alliance', 'Healthcare Retail', '10-K FY2023 (Oct 2023)', 'CIK 0001618921', 'WBA Credit Agreement (2021) §6.1 — Note 8'],
        ['PARA', 'Paramount Global', 'Media & Entertainment', '10-K FY2023 (Feb 2024)', 'CIK 0000813828', 'PARA Credit Agreement (2021) §7.01 — Note 9'],
        ['HCA',  'HCA Healthcare', 'Healthcare Hospitals', '10-K FY2023 (Feb 2024)', 'CIK 0000860731', 'HCA Senior Secured Credit Facilities (2023) §7.1'],
        ['ATUS', 'Altice USA (CSC Holdings)', 'Cable & Telecom', '10-K FY2023 (Mar 2024)', 'CIK 0001672013', 'CSC Holdings Credit Agreement (2019) §7.1'],
        ['BHC',  'Bausch Health Companies', 'Specialty Pharma ★NEW', '10-K FY2023 (Mar 2024)', 'CIK 0000885590', 'BHC Secured Credit Agreement (2022) §7.1 — Note 11'],
        ['M',    "Macy's Inc.", 'Department Store Retail ★NEW', '10-K FY2023 (Feb 2024)', 'CIK 0000794367', "Macy's Credit Agreement (2023 amendment) §7.1"],
        ['LUMN', 'Lumen Technologies', 'Enterprise Fiber Telecom ★NEW', '10-K FY2023 (Mar 2024)', 'CIK 0000018926', 'Lumen Credit Agreement (2022 amendment) §7.1 — Note 7'],
    ]
    cw = [CONTENT_W * x for x in [0.07, 0.18, 0.17, 0.17, 0.12, 0.29]]
    disc_tbl = Table(disc_data, colWidths=cw, repeatRows=1)
    disc_tbl.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), NAVY),
        ('TEXTCOLOR', (0,0), (-1,0), WHITE),
        ...
        ('VALIGN', (0,0), (-1,-1), 'TOP'),
        # Highlight new rows
        ('BACKGROUND', (0,6), (-1,8), colors.HexColor('#EEF4FF')),
        ('TEXTCOLOR', (2,6), (2,8), BLUE),
    ...
    story.append(Table([[
        Paragraph(
            '<b>DISCLAIMER</b><br/><br/>'
            'This report is produced by the DDCSE engine v3.0 for portfolio monitoring and analytical '
            'purposes only. All financial data is sourced from publicly available SEC EDGAR filings '
            'as noted above. Covenant thresholds reflect publicly disclosed credit agreement '
            'terms or representative private credit equivalents. This document does not constitute '
            'investment advice, a solicitation to buy or sell securities, or a credit opinion. '
            'Figures are as of the filing dates noted and may not reflect subsequent developments, '
            'amendments, waivers, or restructuring transactions. Covenant status is as computed '
            'by the DDCSE engine and has not been verified by borrowers, lenders, or their advisors. '
    ...
    story.append(Spacer(1, 4 * mm))
    story.append(Paragraph(
        f'Generated by DDCSE v3.0  ·  github.com/DogInfantry/dynamic-debt-covenant-surveillance-frame  ·  {today}',
        ParagraphStyle('gen', fontName='Helvetica', fontSize=7.5,
            textColor=MGRAY, leading=11, alignment=TA_CENTER)))


def build_stress_section_v3(story):
    """v3 stress section covering all 8 credits."""
    section_header_block(story, '6. Macro Stress Sensitivity Matrices — All 8 Credits',
        'EBITDA compression × total debt increase · NLR sensitivity · Green = Safe · Amber = Watchlist · Red = Breach')

    story.append(Paragraph(
        'Each matrix shows the stressed Net Leverage Ratio under simultaneous EBITDA '
        'compression (0% to –50%) and total debt increases (0% to +30%). '
        'v3.0 extends stress testing to all eight portfolio credits including the three new '
        'sector additions: BHC (Specialty Pharma), M (Retail), and LUMN (Enterprise Telecom).',
        S['body_j']))
    story.append(Spacer(1, 4 * mm))

    all_tickers = ['BHC', 'M', 'LUMN', 'CHTR', 'WBA', 'PARA', 'HCA', 'ATUS']
    pairs = [(all_tickers[i], all_tickers[i+1] if i+1 < len(all_tickers) else None)
    ...
    story.append(Paragraph('Key Stress Scenario Insights — All 8 Credits', S['h3']))
    insights = [
        ('BHC — Bausch Health (NEW · Specialty Pharma)',
         'NLR of 8.84x vs 7.50x — already in deep breach (17.9% breach depth). '
         'Xifaxan LOE risk by 2028 adds a revenue cliff to the existing leverage overhang. '
         '$3.5B TLB maturing June 2025 is the most acute near-term stress event. '
         'Any further EBITDA compression immediately worsens a structurally untenable position.'),
        ("M — Macy's Inc. (NEW · Department Store Retail)",
         'NLR of 1.87x vs 3.50x — ample 46.6% covenant headroom under current ownership. '
         'However, the Arkhouse/Brigade acquisition proposal ($24.80/share) would layer '
         'significant LBO debt onto the structure, potentially pushing pro-forma NLR to 4.5–5.5x '
         'depending on bid financing — a clear covenant breach under current thresholds. '
         'The acquisition risk creates a binary covenant outcome.'),
        ('LUMN — Lumen Technologies (NEW · Enterprise Fiber Telecom)',
         'NLR at 4.51x is marginally above the 4.50x ceiling — effectively on the wall. '
         'FCCR of 0.69x is deeply below 1.00x floor due to $3.2B CapEx fiber build program. '
         'Even a 5% EBITDA compression immediately deepens both breaches. '
         'A second restructuring event is the base-case outcome within 18–24 months.'),
        ('CHTR — Charter Communications',
         'NLR of 4.58x vs 5.00x provides only 8.4% buffer. Cord-cutting acceleration '
         'is the primary macro risk; a 10% EBITDA compression + 5% debt increase would breach.'),
        ('WBA — Walgreens Boots Alliance',
         'Already in breach at 5.40x vs 5.00x. Dependent on covenant waivers and $1B '
         'cost reduction delivery. Any further compression deepens the breach.'),
        ('PARA — Paramount Global',
         'NLR 3.87x vs 4.50x — 16.3% buffer. Skydance merger equity injection (~$8B) '
         'is the deleveraging catalyst. Merger closing risk remains the key watchlist item.'),
        ('HCA — HCA Healthcare',
         'Healthiest credit in the portfolio. NLR 3.10x vs 4.50x — 31.1% buffer. '
         'Declining NLR trend provides additional margin; not stress-vulnerable at any '
         'scenario short of 40% EBITDA compression.'),
        ('ATUS — Altice USA',
         'Deep breach at 8.95x vs 7.00x. Distressed exchange or pre-negotiated '
         'Chapter 11 is base-case. All stress scenarios deepen the breach further.'),
    ]
    for title, text in insights:
        is_new = '★NEW' not in title and 'NEW' in title
        bg_color = colors.HexColor('#EEF4FF') if 'NEW' in title else LGRAY
        story.append(Table([[
            Paragraph(f'<b>{title}:</b> {text}',
                ParagraphStyle('ins', fontName='Helvetica', fontSize=8,
                    textColor=DGRAY, leading=12.5, leftIndent=6)),
...
def build_report_v3(output_path):
    doc = SimpleDocTemplate(
        output_path, pagesize=A4,
        leftMargin=15 * mm, rightMargin=15 * mm,
        topMargin=20 * mm, bottomMargin=14 * mm,
        title='DDCSE Covenant Surveillance Report v3.0',
        author='Anklesh Rawat — DogInfantry',
        subject='Private Credit Covenant Surveillance — 8 Credits / 6 Sectors',
        keywords='covenant surveillance private credit leveraged finance pharma retail telecom',
    )
    ...
    # Section 3 — NEW sectors
    build_new_sector_section(story)

    # Section 4 — Original portfolio
    build_original_sector_section(story)

    # Section 5 — Cascade
    build_cascade_section(story)

    # Section 6 — Stress matrices (all 8)
    build_stress_section_v3(story)

    # Section 7 — Methodology
    build_expanded_methodology(story, today)
...
if __name__ == '__main__':
    output = '/mnt/user-data/outputs/DDCSE_Covenant_Surveillance_Report_v3.pdf'
    build_report_v3(output)
